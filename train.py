...
data_00 = np.load('E:\\cat_hipsterizer-master\\dataset\\CAT_00.npy')
data_01 = np.load('E:\\cat_hipsterizer-master\\dataset\\CAT_01.npy')
data_02 = np.load('E:\\cat_hipsterizer-master\\dataset\\CAT_02.npy')
data_03 = np.load('E:\\cat_hipsterizer-master\\dataset\\CAT_03.npy')
data_04 = np.load('E:\\cat_hipsterizer-master\\dataset\\CAT_04.npy')
data_05 = np.load('E:\\cat_hipsterizer-master\\dataset\\CAT_05.npy')
data_06 = np.load('E:\\cat_hipsterizer-master\\dataset\\CAT_06.npy')

print('dataloads finish!')
print('data preprocessing start!')

# x_train : 학습용 이미지 데이터
# y_train : 학습용 정답 레이블 (Bounding Box 레이블)
# x_test : 테스트용 이미지 데이터
# y_test : 테스트용 정답 레이블 (Bounding Box 레이블)
...
x_train = np.reshape(x_train, (-1, img_size, img_size, 3))  # img_size = 224
...
# include_top = False 최종 Output Layer들을 포함시키지 않고 model load    
# include_top = True로 하면, imagenet을 output으로 내보내게 된다.(즉, 1000개 image들을 분류하는 모델이 된다.)
...
model.fit(x_train, y_train, epochs=50, batch_size=32, shuffle=True,
  ...
  callbacks=[
    TensorBoard(log_dir='E:\\cat_hipsterizer-master\\logs\\%s_bbs' % (start_time)),
    ModelCheckpoint('E:\\cat_hipsterizer-master\\models\\%s_bbs.h5' % (start_time), monitor='val_loss', verbose=1, save_best_only=True, mode='auto'),
